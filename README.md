...
affects simulated agents only.

0.11.0 added readiness work for clients whose processes are short-lived — a status op, an
agent-enforced write allowlist, structured lease-holder diagnostics, and a longer default transport
grace so a per-command process resumes rather than reconnects
