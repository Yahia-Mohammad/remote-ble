...
# tag to also verify the release ref, for example: bash scripts/check-release-version.sh v0.10.0.
set -euo pipefail
...
cargo_lock_version="$(sed -nE '/^name = "agent-rs"$/{n;s/^version = "([0-9]+\.[0-9]+\.[0-9]+)"$/\1/p;}' agent-rs/Cargo.lock)"
...
test -n "$gradle_version" && test -n "$cargo_version" && test -n "$cargo_lock_version" && test -n "$plist_versions" && test -n "$readme_version" && test -n "$kotlin_agent_version"
test "$gradle_version" = "$cargo_version"
test "$gradle_version" = "$cargo_lock_version"
